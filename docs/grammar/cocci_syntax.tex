...
The metavariable section enables to name rules, inherit from others
and define metavariables which will be used for matching in the next
transformation section. The metavariable section also allows to
refine the isomorphisms that should be used during the matching
process.
...
should occur at least once in the transformation immediately following
...
that was declared usgin \texttt{type} and stands for any type.

The \NT{ctype} nonterminal is used by both the grammar of metavariable
...
The grammar of the transformation is not actually the grammar of the
SmPL code that can be written by the programmer, but the grammar of
the slice of this consisting of the {\tt -} annotated and the
unannotated code (the context of the transformed lines), or the {\tt
...
represent at most one match of the given pattern. \mtt{*} is used for
semantic match, \emph{i.e.}, without modification of the matched
code. There are some constraints on the use of these annotations:
\begin{itemize}
