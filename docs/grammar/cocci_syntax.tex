...
\begin{grammar}
  ...
  \CASE{\any{\NT{include}} \opt{\NT{OPTDOTSEQ}(\some{\NT{fun\_decl\_stmt}} \OR \NT{expr}, \NT{when})}}
...
%  \CASE{<+... \any{\NT{when}} \NT{nest\_after\_dots} ...+>}
  ...
  \RULE{\rt{when\_code}}
  \CASE{\NT{OPTDOTSEQ}(\some{\NT{decl\_stmt}}, \NT{when})}
  \CASE{\NT{OPTDOTSEQ}(\NT{expr}, \NT{when})}
...
\begin{grammar}
  \RULE{\rt{OPTDOTSEQ}(\NT{grammar}, \NT{when})}
  \CASE{\opt{... \opt{\NT{when}}} \NT{grammar}
    \ANY{... \opt{\NT{when}} \NT{grammar}}
    \opt{... \opt{\NT{when}}}}
  \CASE{<... \any{\NT{when}} \NT{grammar} ...>}
  \CASE{<+... \any{\NT{when}} \NT{grammar} ...+>}

%  \CASE{}\multicolumn{3}{r}{\hspace{1cm}
%  \KW{\opt{... \opt{\NT{when}}} \NT{grammar}
%    \ANY{... \opt{\NT{when}} \NT{grammar}}
%    \opt{... \opt{\NT{when}}}}
%  }
\end{grammar}
...
\begin{grammar}
  ...
  \CASE{\NT{NEST}(\some{\NT{decl\_stmt}}, \NT{when})}
  \CASE{\NT{NEST}(\NT{expr}, \NT{when})}
  \CASE{break;}
  \CASE{continue;}
  \CASE{\NT{ident}:}
  \CASE{goto \NT{ident};}
...
% \\&\multicolumn{1}{r}{\(\mid\)}&
%   NEST(\some{\mita{decl\_statement}} \(\mid\) \mita{expr}, \mita{when})
% \end{tabular}}
