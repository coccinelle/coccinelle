...
\fixme{Difference between [] for arrays and alternative ?!}
...
\begin{grammar}
  ...
  \CASE{\OPT{! \OR ? \OR +} position \opt{any} \ANY{\T{id} ,} \T{id} ;}
  \CASE{\OPT{! \OR ? \OR +} parameter list [\T{id}] \ANY{\T{id} ,} \T{id} ;}
  \CASE{\OPT{! \OR ? \OR +} expression list [\T{id}] \ANY{\T{id} ,} \T{id} ;}
