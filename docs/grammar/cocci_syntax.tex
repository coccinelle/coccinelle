...
\begin{grammar}
  ...
  \RULE{toplevel\_seq\_start\_after\_dots\_init}
  \CASE{\NT{stmt\_dots} \NT{toplevel\_after\_dots\_init}}
  \CASE{\NT{expr} \NT{toplevel\_after\_exp}}
  ...
  \RULE{nest\_after\_dots}
  \CASE{\NT{decl\_stmt\_exp} \opt{\NT{nest\_after\_stmt}}}
  \CASE{\opt{\NT{exp}} \opt{\NT{nest\_after\_exp}}}

  \RULE{nest\_after\_stmt}
  \CASE{\NT{stmt\_dots} \NT{nest\_after\_dots}}
  \CASE{\NT{decl\_stmt} \opt{\NT{nest\_after\_stmt}}}

  \RULE{nest\_after\_exp}
  \CASE{\NT{stmt\_dots} \NT{nest\_after\_dots}}

  \RULE{toplevel\_after\_dots\_init}
  \CASE{}

  \RULE{toplevel\_after\_exp}
  \CASE{}

  \RULE{decl\_stmt\_expr}
  \CASE{}

  \RULE{toplevel\_after\_stmt}
...
\begin{grammar}
  ...
  \RULE{struct\_decl\_list\_start}
  ...
  \RULE{continue\_struct\_decl\_list}
  \CASE{\NT{struct\_decl} \NT{struct\_decl\_list\_start}}
  \CASE{\NT{struct\_decl}}
...
\begin{lstlisting}[language=C,commentstyle=\color{BrickRed},keywordstyle=\color{blue},stringstyle=\color{OliveGreen}]
#DEFINE TEST "Foo"
