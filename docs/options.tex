...
\title{Revised options for Coccinelle}
...
\noindent
One can either initiate the complete process from step 1.  It is also
possible to perform step 1 or step 2 individually.

Coccinelle has quite a lot of options.  The most basic usages are as
follows, for a semantic match {\tt foo.cocci}, a C file {\tt foo.c}, and a
directory {\tt foodir}:

\begin{itemize}
\item {\tt spatch -parse\_cocci foo.cocci}: Checks that the semantic patch
  is syntactically correct.
\item {\tt spatch -parse\_c foo.c}: Checks that the C file
...
\item {\tt spatch -sp\_file foo.cocci foo.c}: Apply the semantic patch {\tt
    foo.c} to the file {\tt foo.c} and print out any transformations as a
  diff.
...
\item {\tt spatch -sp\_file foo.cocci -dir foodir -include_headers}:  Apply
...
\normal{-parse\_cocci $\langle$file$\rangle$}{ This option parses a semantic
patch file and prints out some information about it.  \jl{add some show
options}}

\subsection{The semantic patch}

\minimum{-c $\langle$file$\rangle$, -sp\_file $\langle$file$\rangle$,
-cocci\_file $\langle$file$\rangle$}{ The provided filename is the name of
the semantic patch, which should end in {\tt .cocci}.}
...
\rare{-ctl\_show\_mcodekind}{ This option
causes transformation information to be shown within the CTL representation
...
file.  This requires {\tt dot} and {\tt gv}.}
...
file tests/compare1.c to the file tests/compare1.c.}
...
Anything that is not associated with an argument keyword or that does not
have the extension {\tt .cocci} is assumed to be a C file or directory to
process.  Normally, only one file or one directory is specified.  If
multiple files are specified, they are treated in parallel, ie the first
rule is applied to all, then the second rule is applied to all, etc.  If a
directory is specified then no files may be specified and only the
rightmost directory specified is used.

\minimum{-dir}{ Deprecated option for specifying a directory.}
...
this option prints regular expression that will be passed to glimpse.}
...
\developer{-testall}{A shortcut for running Coccinelle on all files in a
  subdirectory tests such that there are all of a {\tt .cocci} file, a {\tt
...
\developer{-debug\_cpp, -debug\_lexer, -debug\_etdt, -debug\_typedef,
  -filter\_msg, -filter\_define\_error, -filter\_passed\_level}{Various options
  for debugging the C parser.}
...
\developer{-only\_return\_is\_error\_exit}{In matching ``{\tt{\ldots}}'' in
  ...
  rule.  This is normally relaxed for error handling code.  Normally, error
...
These options control which includes present in a C file are taken into
...
\rare{-relax\_include\_path}{This option causes the search for local
  include files to include the directory specified using {\bf -I}.}
...
bindings resulting from each such application.}
...
\rare{-graphical\_trace}{Create a pdf file containing the control flow
  ...
  is voluminous, and so the option is not really practical for most examples.}
...
Normally, the only output is a diff printed on the screen.
...
\rare{-U}{ Set number of diff context lines.  This option requires that
-show\_diff be set (either explicitly or implicitly).}

\rare{-save\_tmp\_files}{The transformation process creates some temporary
...
\minimum{-version}{ The version of Coccinelle.  No other options are
allowed.}

\rare{-date}{ The date of the current version of Coccinelle. No other
options are allowed.}
