...
A metavariable declaration {\bf local idexpression} v means that v is
restricted to be a local variable.  If it should just be a variable, but
not necessarily a local one, then drop local.  A more complex description
of a location, such as a->b is considered to be an expression, not an
idexpression.
...
rule.  The code must represent an expression must have type bool in the
