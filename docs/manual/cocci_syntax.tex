...
{\tt metavariable}.  That is, the parser must be able to figure out what
...
\begin{itemize}
\item {\tt f(e)}: This only matches the expression {\tt f(e)}.
\item {\tt f(E)}: This matches a call to f with any argument.
\item {\tt F(E)}: This gives a parse error; the semantic patch parser
  cannot figure out what kind of metavariable {\tt F} is.
\item {\tt F:identifier:(E)}: This matches any one argument function call.
\item {\tt F:identifier:(F)}: This matches any one argument function call
...
\item {\tt F:identifier:(F:identifier:)}: This matches any one argument
  function call 
  where the argument is the name of the function itself.  This example
  shows that it is possible to repeat the metavariable type name.
\end{itemize}

When constraints, \textit{e.g.} {\tt when != e}, are allowed but the
expression {\tt e} must be represented as a single token.

The generated semantic patch behaves as though there is a {\tt *} in front
of every token.
