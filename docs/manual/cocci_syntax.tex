...
\item \texttt{F:identifier:(E)}: This matches any one argument function call.
...
When constraints, \textit{e.g.} \texttt{when != e}, are allowed but the
expression \texttt{e} must be represented as a single token.

The generated semantic patch behaves as though there is a \texttt{*} in front
of every token.
