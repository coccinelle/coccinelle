...
the semicolon are only removed if all of the variables are removed.  On the
other hand, it is not possible to write a single rule that removes a
variable from within a declaration of multiple variables and replaces the
declaration by something else.  There is no crash, simply a match failure.
A message is given for this with the {\tt -{}-debug option}.
