...
\developer{-{}-testall}{A shortcut for running Coccinelle on all files in a
  ...
  expected result. Tests whose name ends in "_failure" are
  known to fail and will not affect the total score. This command must be
  run in the root coccinelle folder as it expects a folder named "tests"
  to exist.}
...
\developer{-{}-cpptestall}{A shortcut for running Coccinelle on all files
  ...
  {\tt .res} contains the expected result.  This command must be
