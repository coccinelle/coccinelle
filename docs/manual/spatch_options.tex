...
directory can be specified as well and will be processed recursively.  This
is done using the unix {\tt find} command, which does not follow symbolic
links.  Normally, only one C file or one directory is specified.  If
multiple C files are specified, they are treated in parallel, {\em i.e.}, the
first semantic patch rule is applied to all functions in all files, then
the second semantic patch rule is applied to all functions in all files,
etc.  If a directory is specified then no files may be specified and only
the rightmost directory specified is used.
...
\rare{-dir}{ Specify a directory containing C files to process.  By
  ...
  that always works on a single directory, but allows the user to override
  the provided directory with another one.  If only one directory is to be
  provided, the {\bf -dir} option is not needed.  Spatch will infer that a
  directory argument should be treated recursively.}
