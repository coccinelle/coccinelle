...
\rare{-ifdef\_to\_if,-no_ifdef\_to\_if}{ The option {\bf -ifdef\_to\_if}
represents an \#ifdef in the source code as a conditional in the
control-flow graph when doing so represents valid code.  {\bf
-no\_ifdef\_to\_if} disables this option.  {\bf -ifdef\_to\_if} is the
default.}
