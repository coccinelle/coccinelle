...
\developer{-{}-testall}{A shortcut for running Coccinelle on all files in a
  ...
  ``tests/SCORE\_expected.sexp''. Tests whose name ends in "_failure" are
  known to fail and will not affect the total score.}
...
\developer{-{}-cpptestall}{A shortcut for running Coccinelle on all files
  ...
  {\tt .res} contains the expected result.  If the argument
  -{}-expected-score-file is provided, then that file is used for the
  result.  Otherwise, the result goes in
  ``tests/SCORE\_expected.sexp''. {\bf Warning:} It is intended that not
  all of the test cases provided with Coccinelle actually pass.}
