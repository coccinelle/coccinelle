...
and vk_statement = fun bigf (st: Ast_c.statement) ->
  ...
  and k st =
    ...
    | Iteration  (ScopedGuard (es, st)) ->
        vk_argument_list bigf es; statf st;
...
and vk_type = fun bigf t ->
  ...
  and k t =
    ...
    | TypeName  (name) -> vk_name bigf name
...
and vk_node = fun bigf node ->
  ...
  let k n =
    ...
    | (
        F.TopNode|F.EndNode|
        F.CatchExit|F.ErrorExit|F.Exit|F.Enter|F.LoopFallThroughNode|F.FallThroughNode|
...
and vk_name_s = fun bigf ident ->
  ...
  and k id =
    ...
    | QualName xs ->
        QualName (xs +> List.map (fun (nm, ii2) ->
          (namef nm), iif ii2
        ))
    | CppConcatenatedName xs ->
...
and vk_statement_s = fun bigf st ->
  ...
  and k st =
    ...
    let st' =
	  ...
	  let es =
	    es +>
	    List.map (fun (e,ii) -> vk_argument_s bigf e, vk_ii_s bigf ii) in
          Iteration  (ScopedGuard (es, statf st))
      | Iteration (While (first, st))    ->
	(match first with
	  WhileExp(e) ->
...
and vk_type_s = fun bigf t ->
  ...
  and k t =
    ...
    let t' =
      ...
      | TypeName name -> TypeName (vk_name_s bigf name)
...
and vk_attribute_s = fun bigf attr ->
  ...
  | CxxAttribute args, ii ->
      CxxAttribute (args +> List.map (fun (e,ii) ->
        vk_argument_s bigf e, iif ii)), iif ii
  | CxxAttributeUsing (atnm, args), ii ->
      CxxAttributeUsing (
        vk_name_s bigf atnm,
...
and vk_node_s = fun bigf node ->
  ...
  and k node =
    ...
    | F.ScopedGuardHeader (st, (es,ii)) ->
	let es =
	  es +>
	  List.map (fun (e,ii) -> vk_argument_s bigf e, iif ii) in
	F.ScopedGuardHeader (st, (es, iif ii))
    | F.TryHeader (st,info) ->
