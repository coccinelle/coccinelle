...
let rec expression e =
  ...
  | Ast0.QualifiedAccess(None,coloncolon,field) ->
      let coloncolon = normal_mcode coloncolon in
      let field = ident field in
      mkres e (Ast0.QualifiedAccess(None,coloncolon,field))
        (promote_mcode coloncolon) field
  | Ast0.QualifiedAccess(Some ty,coloncolon,field) ->
      let ty = typeC ty in
      let coloncolon = normal_mcode coloncolon in
      let field = ident field in
      mkres e (Ast0.QualifiedAccess(Some ty,coloncolon,field))
          ty field
  ...
  | Ast0.TupleExpr(init) ->
      ...
      mkres e (Ast0.TupleExpr(init)) init init
  | Ast0.TypeExp(ty) ->
...
and field d =
  ...
  | Ast0.AccSpec(decl,dd) ->
      ...
      mkres d (Ast0.AccSpec(decl,dd)) l r
  | Ast0.OptField(decl) ->
...
and directive d =
  match Ast0.unwrap d with
    Ast0.Include(inc,stm) ->
      ...
      mkres d (Ast0.Include(inc,stm)) (promote_mcode inc) (promote_mcode stm)
  | Ast0.MetaInclude(inc,s) ->
      let inc = normal_mcode inc in
      let s = expression s in
      mkres d (Ast0.MetaInclude(inc,s)) (promote_mcode inc) s
...
and attribute attr =
  ...
  | Ast0.GccAttribute(attr_,lp1,lp2,args,rp1,rp2) ->
      ...
      let args = dots is_exp_dots (Some(promote_mcode lp2)) expression args in
      let rp1 = normal_mcode rp1 in
      let rp2 = normal_mcode rp2 in
      let ln1 = promote_mcode attr_ in
      let ln2 = promote_mcode rp2 in
      mkres attr (Ast0.GccAttribute(attr_,lp1,lp2,args,rp1,rp2)) ln1 ln2
  | Ast0.CxxAttribute(lb1,args,rb1,rb2) ->
      let lb1 = normal_mcode lb1 in
      let args = dots is_exp_dots (Some(promote_mcode lb1)) expression args in
      let rb1 = normal_mcode rb1 in
      let rb2 = normal_mcode rb2 in
      let ln1 = promote_mcode lb1 in
      let ln2 = promote_mcode rb2 in
      mkres attr (Ast0.CxxAttribute(lb1,args,rb1,rb2)) ln1 ln2
  | Ast0.CxxAttributeUsing(lb1,usng,atnm,dotdot,args,rb1,rb2) ->
      ...
      let ln1 = promote_mcode lb1 in
      let ln2 = promote_mcode rb2 in
      mkres attr (Ast0.CxxAttributeUsing(lb1,usng,atnm,dotdot,args,rb1,rb2)) ln1 ln2
  ...
  let res =
	...
	let first =
	  ...
	  | Ast0.ForRange((_,bef),decl,ini) ->
	      ...
	      let (leftinfo,decl) = promote_to_statement_start decl bef in
	      let ini = initialiser ini in
	      mkres first (Ast0.ForRange ((leftinfo,bef),decl,ini)) decl ini in
	let rp = normal_mcode rp in
	let body = statement body in
	let (rightinfo,right,body) = promote_to_statement_end body aft in
