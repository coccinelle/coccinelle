...
let dot_list fn l =
  ...
  (l,List.hd l,List.hd (List.rev l))

let dots fn d =
  match Ast0.unwrap d with
    Ast0.DOTS(x) ->
      let (l,lstart,lend) = dot_list fn x in
      mkres (Ast0.DOTS(List.map fn x)) lstart lend
  | Ast0.CIRCLES(x) ->
      let (l,lstart,lend) = dot_list fn x in
      mkres (Ast0.CIRCLES(List.map fn x)) lstart lend
  | Ast0.STARS(x) ->
      let (l,lstart,lend) = dot_list fn x in
      mkres (Ast0.STARS(List.map fn x)) lstart lend
...
let rec expression e =
  ...
  | Ast0.NestExpr(exp_dots) ->
      let exp_dots = dots expression exp_dots in
...
let rec statement s =
  ...
  | Ast0.Disj(rule_elem_dots_list) ->
      let elems =
	List.map (function x -> dots statement x) rule_elem_dots_list in
      mkres (Ast0.Disj(elems)) (List.hd elems) (List.hd (List.rev elems))
  | Ast0.Nest(rule_elem_dots) ->
      let rule_elem_dots = dots statement rule_elem_dots in
...
let top_level = function
  ...
  | Ast0.CODE(rule_elem_dots) -> Ast0.CODE(dots statement rule_elem_dots)
