...
let rewrap (_,l,fvs,mfvs,fresh,inherited,saved,d,pos,isos) x =
  (x,l,fvs,mfvs,fresh,inherited,saved,d,pos,isos)
let rewrap_mcode (_,a,b) x = (x,a,b)
let unwrap (x,_,_,_,_,_,_,_,_,_) = x
let unwrap_mcode (x,_,_) = x
let get_mcodekind (_,_,x) = x
let get_line (_,l,_,_,_,_,_,_,_,_) = l
let get_mcode_line (_,l,_) = l.line
let get_fvs (_,_,fvs,_,_,_,_,_,_,_) = fvs
let set_fvs fvs (x,l,_,mfvs,fresh,inherited,saved,d,pos,isos) =
  (x,l,fvs,mfvs,fresh,inherited,saved,d,pos,isos)
let get_mfvs (_,_,_,mfvs,_,_,_,_,_,_) = mfvs
let set_mfvs mfvs (x,l,fvs,_,fresh,inherited,saved,d,pos,isos) =
  (x,l,fvs,mfvs,fresh,inherited,saved,d,pos,isos)
let get_fresh (_,_,_,_,fresh,_,_,_,_,_) = fresh
let get_inherited (_,_,_,_,_,inherited,_,_,_,_) = inherited
let get_saved (_,_,_,_,_,_,saved,_,_,_) = saved
let get_dots_bef_aft (_,_,_,_,_,_,_,d,_,_) = d
let set_dots_bef_aft d (x,l,fvs,mfvs,fresh,inherited,saved,_,pos,isos) =
  (x,l,fvs,mfvs,fresh,inherited,saved,d,pos,isos)
let get_pos (_,_,_,_,_,_,_,_,pos,_) = pos
let rewrap_dots_bef_aft (x,l,fvs,mfvs,fresh,inherited,saved,_,pos,isos) d =
  (x,l,fvs,mfvs,fresh,inherited,saved,d,pos,isos)
let rewrap_pos (x,l,fvs,mfvs,fresh,inherited,saved,d,_,isos) pos =
  (x,l,fvs,mfvs,fresh,inherited,saved,d,pos,isos)
let get_isos (_,_,_,_,_,_,_,_,_,isos) = isos
