...
let do_get_constants constants keywords env (neg_pos,_) =
  ...
  let attribute r k a =
    ...
    | Ast.GccAttribute(attr_,_,_,arg,_,_) ->
        bind (keywords (Ast.unwrap_mcode attr_)) (k a)
    | Ast.CxxAttribute(_,arg,_,_) -> k a
    | Ast.CxxAttributeUsing(_,_,_,_,arg,_,_) -> k a in
  ...
  let declaration r k d =
    ...
    | Ast.Init(_,_,_,_,endattr,_,_,_) | Ast.UnInit(_,_,_,_,endattr,_) ->
	List.fold_left bind (k d)
	  (List.map r.V.combiner_attribute endattr)
    | _ -> k d in
