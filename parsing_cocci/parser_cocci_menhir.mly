...
struct_decl_one:
    ...
    | fas=TPrivate dd=TDotDot
         { Ast0_cocci.wrap(Ast0_cocci.AccSpec(Parse_aux.clt2mcode "private" fas, Parse_aux.clt2mcode ":" (snd dd))) }
    | fas=TProtected dd=TDotDot
         { Ast0_cocci.wrap(Ast0_cocci.AccSpec(Parse_aux.clt2mcode "protected" fas, Parse_aux.clt2mcode ":" (snd dd))) }
    | fas=TPublic dd=TDotDot
         { Ast0_cocci.wrap(Ast0_cocci.AccSpec(Parse_aux.clt2mcode "public" fas, Parse_aux.clt2mcode ":" (snd dd))) }
...
decl: t=ctype d=direct_declarator(type_ident) endar=attr_list
    ...
    | t=ctype
        { (*verify in FunDecl*)
          Ast0_cocci.wrap(Ast0_cocci.Param(t, None, [])) }
    | TMetaParam
...
decl_var:
    t=ctype pv=TPtVirg
      { [Ast0_cocci.wrap
	    (Ast0_cocci.TyDecl(t,Parse_aux.clt2mcode ";" (snd pv)))] }
  ...
  | f=funproto { [f] }
  | al=ioption(alignas) s=ioption(storage) t=ctype
      d=direct_declarator(type_ident)
      endattrs=attr_list q=TEq e=initialize pv=TPtVirg
      {let (id,fn) = d in
      [Ast0_cocci.wrap
	  (Ast0_cocci.Init
  ...
  | al=ioption(alignas) s=ioption(storage) cv=const_vol
      ...
      midattrs=const_vol_attr_list d=comma_list(d_ident) pv=TPtVirg
	  ...
	  (function (id,fn) ->
	    ...
	    Ast0_cocci.wrap
	      (Ast0_cocci.UnInit
		 (al,s,fn idtype,id,[],Parse_aux.clt2mcode ";" (snd pv))))
	  d }
  | al=ioption(alignas) s=ioption(storage) cv=const_vol i=ident_or_template_type
      midattrs=const_vol_attr_list d=d_ident endattrs=attr_list q=TEq e=initialize
      pv=TPtVirg
      { let cv = List.map (fun x -> Ast0.CV x) cv in
        let (id,fn) = d in
	let idtype = Parse_aux.make_cv cv i midattrs in
  ...
  | s=ioption(storage) prear=attr_list d=decl_ident
      ...
      { [Ast0_cocci.wrap
	    (Ast0_cocci.MacroDecl(s,prear,d,Parse_aux.clt2mcode "(" o,e,
				  Parse_aux.clt2mcode ")" c,ar,
				  Parse_aux.clt2mcode ";" (snd p)))] }
  | s=ioption(storage) prear=attr_list
      d=decl_ident o=TOPar e=eexpr_list_option c=TCPar ar=attr_list q=TEq
      i=initialize p=TPtVirg
      { [Ast0_cocci.wrap
  ...
  | s=Ttypedef t=typedef_ctype
	  ...
	  (function (id,fn) ->
	    Ast0_cocci.wrap
	      (Ast0_cocci.Typedef(s,fn t,id,Parse_aux.clt2mcode ";" (snd pv))))
	  d }
...
cast_expr(r,pe):
  ...
  | lp=TOPar t=ctype d=direct_abstract_d rp=TCPar e=cast_expr(r,pe)
      { Ast0_cocci.wrap(Ast0_cocci.Cast (Parse_aux.clt2mcode "(" lp, d t,
			     Parse_aux.clt2mcode ")" rp, e)) }
