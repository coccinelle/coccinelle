...
let rec equal_ident i0 i1 =
  ...
  | MetaId (mn0, id0, s0, p0), MetaId (mn1, id1, s1, p1) ->
      unwrap_mcode mn0 = unwrap_mcode mn1 &&
      id0 = id1 && s0 = s1 && p0 = p1
  | MetaFunc (mn0, id0, p0), MetaFunc (mn1, id1, p1)
  | MetaLocalFunc (mn0, id0, p0), MetaLocalFunc (mn1, id1, p1) ->
      unwrap_mcode mn0 = unwrap_mcode mn1 &&
      id0 = id1 && p0 = p1
  | AsIdent (i0a, i0b), AsIdent (i1a, i1b) ->
      equal_ident i0a i1a && equal_ident i0b i1b
  | DisjId (_, l0, _, _), DisjId (_, l1, _, _) ->
      List.for_all2 equal_ident l0 l1
  | OptIdent i0, OptIdent i1 -> equal_ident i0 i1
  | _, _ -> false
...
let rec type_compatible ty0 ty1 =
  ...
  | StructUnionName (su0, i0), StructUnionName (su1, i1) ->
      ...
      Common.equal_option equal_ident i0 i1
  | MetaType (mn0, p0), MetaType (mn1, p1) ->
      unwrap_mcode mn0 = unwrap_mcode mn1 &&
      p0 = p1
  | AsType (ty0a, ty0b), AsType (ty1a, ty1b) ->
      type_compatible ty0a ty1a &&
      type_compatible ty0b ty1b
  | DisjType (_, l0, _, _), DisjType (_, l1, _, _) ->
      List.for_all2 type_compatible l0 l1
  | _, _ -> false
