...
let collect_all_refs =
  ...
  let astfvexpr recursor k e =
    ...
    | Ast.MetaExprList(name,lenname,_,_) -> [metaid name;lenname]
  ...
  let astfvparam recursor k p =
    ...
    | Ast.MetaParamList(name,lenname,_,_) -> [metaid name;lenname]
    | _ -> k p in
...
let collect_saved =
  ...
  let astfvexpr recursor k e =
	...
	Ast.MetaErr(name,TC.Saved,_) | Ast.MetaExpr(name,TC.Saved,_,_,_) ->
	  [metaid name]
      | Ast.MetaExprList(name,lenname,TC.Saved,_) -> [metaid name;lenname]
      | _ -> k e in
  ...
  let astfvparam recursor k p =
    ...
    | Ast.MetaParamList(name,lenname,TC.Saved,_) -> [metaid name;lenname]
    | _ -> k p in
