	...
	then
	  ...
	  | (Ast0.TemplateInst(namea,laba,argsa,raba),Ast0.TemplateInst(nameb,labb,argsb,rabb)) ->
		...
		[check_mcode laba labb; check_mcode raba rabb; match_expr namea nameb;
		  match_dots match_expr is_elist_matcher do_elist_match
		    argsa argsb]
	| (Ast0.TupleExpr(inita),Ast0.TupleExpr(initb)) ->
...
(* the special case for function types prevents the eg T X; -> T X = E; iso
  ...
  and match_attribute a1 a2 =
    ...
    | (Ast0.GccAttribute(attr_1,lp11,lp21,args1,rp11,rp21),
       Ast0.GccAttribute(attr_2,lp12,lp22,args2,rp12,rp22)) ->
	       conjunct_many_bindings
    ...
    | (Ast0.CxxAttribute(lb1,args1,rb1,rb1a),
       Ast0.CxxAttribute(lb2,args2,rb2,rb2a)) ->
	       conjunct_many_bindings
		 [check_mcode lb1 lb2; check_mcode rb1 rb2; check_mcode rb1a rb2a;
		   match_dots match_expr is_elist_matcher do_elist_match args1 args2]
    | (Ast0.CxxAttributeUsing(lb1,usng1,atnm1,dotdot1,args1,rb1,rb1a),
       Ast0.CxxAttributeUsing(lb2,usng2,atnm2,dotdot2,args2,rb2,rb2a)) ->
	       conjunct_many_bindings
		 [check_mcode lb1 lb2; check_mcode rb1 rb2; check_mcode rb1a rb2a;
...
let extra_copy_stmt_plus model e =
  ...
  then
    ...
    | Ast0.ScopedGuard(_,_,_,_,_,(_,aft,_)) ->
	(match Ast0.unwrap e with
	...
	| Ast0.ScopedGuard(_,_,_,_,_,(_,aft1,_))->
