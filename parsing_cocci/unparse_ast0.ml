...
and typeC t =
  print_context t
	...
	Ast0.ConstVol(cvbefore,ty,cvafter) ->
	  ...
	  typeC tn; mcode print_string_box lp;
	  let _ = dots (function _ -> ()) expression args in
	  close_box(); mcode print_string rp
...
and field d =
  print_context d
	...
	Ast0.MetaField(name,_,_)
      ...
      | Ast0.AccSpec(decl,dd) -> mcode print_string decl; mcode print_string dd
...
and statement arity s =
  print_context s
	...
	Ast0.FunDecl(_,fninfo,name,lp,params,va,rp,attrs,lbrace,body,rbrace,_) ->
	  ...
	  mcode (function _ -> ()) ((),(),info,aft,ref [],adj)
      | Ast0.ScopedGuard(sg,lp,exps,rp,body,(info,aft,adj)) ->
	  print_string arity;
	  mcode print_string sg; print_string " "; mcode print_string_box lp;
	  let _ = dots (function _ -> ()) expression exps in
...
and print_define_param param =
  ...
  | Ast0.DParamEll(id,dots) -> ident id; mcode print_string dots
