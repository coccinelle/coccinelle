 ...
 *)
let make_pos (_, arity, info, mcodekind, _, adj) snp : Ast0.anything * Snap.t =
...
(* TODO: fix the disjunction item. Usually we don't want to put positions
...
let mcode_pos ((x, a, info, mc, pos, q) as mco) snp : 'a Ast0.mcode * Snap.t =
  if a = Ast0.OPT
  then (mco, snp)
  else
...
(* helper for adding mcode position, reconstructs component, always Some *)
let mcode
  ~mc           (* 'a Ast0.mcode *)
...
let assign_pos a snp : Ast0.base_assignOp Ast0.wrap * Snap.t =
...
let binary_pos a snp : Ast0.base_binaryOp Ast0.wrap * Snap.t =
  match Ast0.unwrap a with
  | Ast0.Arith mc ->
 ...
 * Always possible! but not done in optional expressions; ie. arity = Ast0.OPT.
 *)
let rec ident_pos i snp : Ast0.base_ident Ast0.wrap * Snap.t =
...
let wrap (a : 'a) (snp : Snap.t) : ('a Ast0.wrap * Snap.t) option =
 ...
 *)
let item_pos ~item ~item_posfn ~constructor ?(alt = fun _ -> None) snp
...
let rec expression_pos exp snp : (Ast0.expression * Snap.t) option =
   ...
   *)
  let exp_pos ~exp ~constructor ?(alt = fun _ -> None) snp =
    match expression_pos exp snp with
    | Some (exp, snp) -> wrap (constructor ~exp) snp
    | None -> alt() in

  (* try adding a position to internal expressions, first exp1, then exp2, then
   * alt function.
   *)
  let exp_pos2 ~exp1 ~exp2 ~constructor ?(alt = fun _ -> None) snp =
    match expression_pos exp1 snp with
    | Some (exp1, snp) -> wrap (constructor ~exp1 ~exp2) snp
    | None ->
        (match expression_pos exp2 snp with
         | Some (exp2, snp) -> wrap (constructor ~exp1 ~exp2) snp
         | None -> alt()
        ) in
  ...
  | Ast0.FunCall(exp, lp, expdots, rp) ->
      ...
      exp_pos ~exp ~constructor:(c ~mc:lp) ~alt snp
  | Ast0.Assignment(exp1, asop, exp2, st) ->
      let c ~exp1 ~exp2 ~mc = Ast0.Assignment(exp1, mc, exp2, st) in
      let alt() =
        let (mc, snp) = assign_pos asop snp in
        wrap (c ~exp1 ~exp2 ~mc) snp in
      exp_pos2 ~exp1 ~exp2 ~constructor:(c ~mc:asop) ~alt snp
  | Ast0.Sequence(exp1, com, exp2) ->
      ...
      exp_pos2 ~exp1 ~exp2 ~constructor:(c ~mc:com) ~alt snp
  ...
  | Ast0.Unary(exp, unmc) ->
      ...
      exp_pos ~exp ~constructor:(c ~mc:unmc) ~alt snp
  | Ast0.Binary(exp1, bin, exp2) ->
      let c ~exp1 ~exp2 ~mc = Ast0.Binary(exp1, mc, exp2) in
      let alt() =
        let (mc, snp) = binary_pos bin snp in
        wrap (c ~exp1 ~exp2 ~mc) snp in
      exp_pos2 ~exp1 ~exp2 ~constructor:(c ~mc:bin) ~alt snp
  ...
  | Ast0.Cast(lp, typec, rp, exp) ->
      ...
      exp_pos ~exp ~constructor:(c ~mc:rp) ~alt snp
  ...
  | Ast0.SizeOfType(sizeofmc, lp, typec, rp) ->
      ...
      mcode_wrap ~mc:sizeofmc ~constructor snp
  | Ast0.TypeExp(typec) ->
      type_pos typec snp (* sanity check, always returns None *)
  ...
  | Ast0.UniqueExp exp ->
      ...
      exp_pos ~exp ~constructor snp
  | Ast0.OptExp exp ->
      let constructor ~exp = Ast0.OptExp exp in
      exp_pos ~exp ~constructor snp

(* redefine exp_pos outside scope of expression_pos due to internal exp_pos
 * being typed to only work for expression constructors and not 'a constructors
 *)
let exp_pos ~exp ~constructor ?(alt = fun _ -> None) snp =
  let constructor ~item = constructor ~exp:item in
  item_pos ~item:exp ~item_posfn:expression_pos ~constructor ~alt snp

let rec declaration_pos decl snp : (Ast0.declaration * Snap.t) option =
  ...
  | Ast0.OptDecl(dec) ->
      ...
      item_pos ~item:dec ~item_posfn:declaration_pos ~constructor snp
  | Ast0.UniqueDecl(dec) ->
...
let forinfo_pos f snp : (Ast0.forinfo * Snap.t) option =
  ...
  | Ast0.ForExp (Some exp, sem) ->
      ...
      exp_pos ~exp ~constructor:(c ~mc:sem) ~alt snp
  ...
  | Ast0.ForDecl (bef, decl) ->
      ...
      item_pos ~item:decl ~item_posfn:declaration_pos ~constructor snp

let rec statement_pos s snp : (Ast0.statement * Snap.t) option =
  ...
  | Ast0.OptStm stm ->
      ...
      item_pos ~item:stm ~item_posfn:statement_pos ~constructor:c snp
  | Ast0.UniqueStm stm ->
  ...
  | Ast0.Decl (bef, decl) ->
      ...
      item_pos ~item:decl ~item_posfn:declaration_pos ~constructor:c snp
  ...
  | Ast0.IfThen (ifm, l, exp, r, st, a) ->
      ...
      exp_pos ~exp ~constructor:(c ~mc:ifm) ~alt snp
  ...
  | Ast0.While (whmc, l, exp, r, s, a) ->
      ...
      exp_pos ~exp ~constructor:(c ~mc:whmc) ~alt snp
  | Ast0.Do (d, s, whmc, l, exp, r, sem) ->
      ...
      exp_pos ~exp ~constructor:(c ~mc:d) ~alt snp
  | Ast0.For (fo,lp, fi,expo1,sem,expo2,rp,stmt,a) ->
      ...
      item_pos ~item:fi ~item_posfn:forinfo_pos ~constructor:(c ~mc:fo) ~alt snp
  ...
  | Ast0.Switch (sw, lp, exp, rp, lb, sd, cd, rb) ->
      ...
      exp_pos ~exp ~constructor:(c ~mc:sw) ~alt snp
  | Ast0.Label (id,col) ->
  ...
  | Ast0.ReturnExpr (retmc, exp, sem) ->
      ...
      exp_pos ~exp ~constructor:(c ~mc:retmc) ~alt snp
