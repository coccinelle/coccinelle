...
\begin{itemize}
\item \texttt{<name>.c}: \texttt{C} source files that return matches/patches for the corresponding cocci file. These can be tested with \texttt{spatch --sp-file <name>.(sgencocci|cocci) <name>.c}.
...
\item \texttt{<name>.sgencocci}: expected output when running \texttt{sgen} on the unhardened Coccinelle script with the config file. Should be a valid, hardened Coccinelle script.
\end{itemize}
