...
let group_patch patch = 
  let patch' = patch +> List.map (function (CP.File (s, header, body)) -> 
    ...
    | (dir_elems,x)::xs -> 
        if List.length dir_elems >= 2 then
          ...
          let (yes, no) = xs +> Common.partition_either (fun (dir_elems', x) ->
            if List.length dir_elems' >= 2 && 
               Common.take 2 dir_elems' = base
            then Left x
            else Right (dir_elems', x)
          ) in
          (Common.join "/" base, (x::yes))::aux_patch no
        else 
          (Common.join "/" dir_elems, [x])::aux_patch xs
  in
  aux_patch patch'
...
let split_patch str bodymail prefix file = 
  let patch = CP.parse_patch file in
...
let main () = 
  begin
    ...
    | [patch;prefix;str;bodymail] -> split_patch str bodymail prefix patch
