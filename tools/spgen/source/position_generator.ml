...
: (Ast0.base_typeC Ast0.wrap * Snap.t) option =
  ...
  | Ast0.StructUnionDef(t,lcb,decldots,rcb) ->
      ...
      let alt() = mcode_wrap ~mc:rcb ~constructor:(c ~item:t) snp in
      item_wrap ~item:t ~item_posfn:type_pos ~constructor:(c ~mc:rcb) ~alt snp
  | Ast0.TypeName(typename,nm) ->
      let constructor ~id = Ast0.TypeName(typename, id) in
      id_wrap ~id:nm ~constructor snp
...
: (Ast0.base_declaration Ast0.wrap * Snap.t) option =
  ...
  | Ast0.AsDecl _ ->
      None
  | Ast0.Init(al, st, ty, id, endattr, eq, ini, sem) ->
      let _ = type_pos ty snp in (* sanity check *)
      let constructor ~id = Ast0.Init(al, st, ty, id, endattr, eq, ini, sem) in
      id_wrap ~id ~constructor snp
  | Ast0.UnInit(al, st, ty, id, endattr, sem) ->
      let _ = type_pos ty snp in (* sanity check *)
      let constructor ~id = Ast0.UnInit(al, st, ty, id, endattr, sem) in
      id_wrap ~id ~constructor snp
  | Ast0.TyDecl (t, sem) ->
  ...
  | Ast0.MacroDecl (st,pa,id,lp,ed,rp,attr,sem) ->
      let constructor ~id = Ast0.MacroDecl (st, pa, id, lp, ed, rp, attr, sem) in
      id_wrap ~id ~constructor snp
  | Ast0.MacroDeclInit (st,pa,id,lp,ed,rp,attr,eq,init,sem) ->
      let constructor ~id = Ast0.MacroDeclInit (st,pa,id,lp,ed,rp,attr,eq,init,sem) in
      id_wrap ~id ~constructor snp
  | Ast0.OptDecl(dec) ->
...
: (Ast0.base_field Ast0.wrap * Snap.t) option =
  ...
  | Ast0.AccSpec (mc,dd) ->
      let constructor ~mc = Ast0.AccSpec (mc,dd) in
      mcode_wrap ~mc ~constructor snp
  | Ast0.OptField(dec) ->
...
: (Ast0.base_forinfo Ast0.wrap * Snap.t) option =
  ...
  | Ast0.ForExp (Some exp,sem1,expo1,sem2,expo2) ->
      let c ~exp ~mc = Ast0.ForExp(Some exp,mc,expo1,sem2,expo2) in
      let alt() = mcode_wrap ~mc:sem1 ~constructor:(c ~exp) snp in
      exp_wrap ~exp ~constructor:(c ~mc:sem1) ~alt snp
  | Ast0.ForExp (None,sem1,expo1,sem2,expo2) ->
      let constructor ~mc = Ast0.ForExp (None,mc,expo1,sem2,expo2) in
      mcode_wrap ~mc:sem1 ~constructor snp
  | Ast0.ForDecl (bef,decl,expo1,sem,expo2) ->
      let constructor ~item = Ast0.ForDecl(bef,item,expo1,sem,expo2) in
      item_wrap ~item:decl ~item_posfn:declaration_pos ~constructor snp
  | Ast0.ForRange (bef,decl,inio) ->
      let constructor ~item = Ast0.ForRange(bef,item,inio) in
      item_wrap ~item:decl ~item_posfn:declaration_pos ~constructor snp
