...
(* opti: avoid all those rev, and avoid the intervise (but yes the algo are then more complex :) *)
(* opti: balanced set intervalle *)
    ...
    (*  invariant= ordered list, no incoherent interv (one elem or zero elem), merged (intervalle are separated) *)
    let invariant xs = 
      let rec aux min xs = 
        xs +> List.fold_left (fun min e -> 
          match e with 
          | Exact i -> 
              let _ = if i <= min then pr2 (sprintf "i = %d, min = %d" i min) in
              (* todo: should be even stronger, shoud be i > min+1 *)
              let _ = assert (i > min) in
              i
          | Interv (i,j) -> 
              let _ = assert (i > min) in
              let _ = assert (j > i) in
...
(*     union_set (tolist xs) (tolist ys) +> fromlist *)
   ...
   (*  bug/feature:   discovered by vlad rusu, my invariant for intervalle is not very strong, should return (Interv (1,4)) *)
