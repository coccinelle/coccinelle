...
(* graph2way  prend en parametre le type de finitemap et set a prendre
 ...
 TODO efficient graph: with pointers
    and a tag: visited => need keep global value visited_counter
   check(that node is in, ...), display
 pourrait remettre val nods, a la place de les calculer. mais bon s'en sert pas vraiment car
  y'a pas de notion d'identifiant de noeud et de label.
 invariant: key in pred is also in succ (completness) and value in either table is a key also
*)
...
class ['a] ograph2way asucc apred (*f1*) f2 = 
  object(o)
    ...
    method del_node e = {< (* nods = nods#del e; *)
                           ...
                           succ = succ#delkey e;
                          >}
    method add_arc (a,b) = {< succ = succ#replkey (a, (succ#find a)#add b);
                              pred = pred#replkey (b, (pred#find b)#add a);
                             >}
    method del_arc (a,b) = {< succ = succ#replkey (a, (succ#find a)#del b);
    ...
    method nodes = (* nods *)
        ...
        (* caml typing sux, arrive pas a faire:  pred#fold (fun a (k,v) -> a#add k) (new osetb Setb.empty)  *)
        let a = ref (new osetb Setb.empty) in
