...
module type INCREMENTAL_ENGINE = sig
  ...
  type 'result element_elim =
    { element_elim : 'a . 'a lr1state * 'a * position * position -> 'result }

  type element_wrapper =
    { element_wrapper : 'a . 'a element_elim -> 'a }

  type element =
    | Element of element_wrapper
...
module type SYMBOLS = sig
  ...
  type xsymbol_wrapper = { xsymbol_wrapper : 'a 'b . ('a symbol -> 'b) -> 'b }

  type xsymbol =
    | X of xsymbol_wrapper
