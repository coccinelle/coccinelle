...
let rec reduction_path_exists s (w : Symbol.t list) prod : bool =
  ...
  | a :: w ->
      Symbol.non_error a &&
      match try Some (SymbolMap.find a (Lr1.transitions s)) with Not_found -> None with
      | Some s ->
...
module Make (X : sig end) = struct
  ...
  let rec insert (t : trie) (w : Symbol.t list) prod : unit =
    ...
    | a :: w ->
        match try Some (SymbolMap.find a (Lr1.transitions t.current)) with Not_found -> None with
        | None ->
            assert false
        | Some successor ->
